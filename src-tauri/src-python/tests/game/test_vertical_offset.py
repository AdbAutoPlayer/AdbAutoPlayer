...
Covers `_ScreenshotMixin._apply_vertical_offset` (screenshot-space correction)
and `_InputMixin._apply_vertical_offset` (its inverse, applied before
tapping/swiping/holding on the real device).
"""
...
class TestScreenshotVerticalOffset:
    """Tests for `_ScreenshotMixin._apply_vertical_offset`."""
    ...
    def test_zero_offset_returns_image_unchanged(self):
        ...
        with patch(
            ...
            result = _ScreenshotMixin._apply_vertical_offset(image)
