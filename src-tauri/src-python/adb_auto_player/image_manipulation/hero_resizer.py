...
def resize_hero_template(file_path, scale_factor=1/1.75):
    ...
    print(f"Successfully resized {os.path.basename(file_path)} to {new_width}x{new_height}")
    return True
...
def find_optimal_scale(template_path, serial="127.0.0.1:5555", screenshot_path=None):
    ...
    for scale in scales:
        ...
        result = matcher.find_template_match(ss, resized_tpl, threshold=ConfidenceValue('10%'))
    ...
    if best_scale:
        print(f"Optimal scale found: {best_scale:.3f} (Confidence: {max_confidence*100:.1f}%)")
        return best_scale
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Resize hero templates for AFK Journey automation.")
    parser.add_argument("file", help="Path to the hero .png template")
    parser.add_argument("--scale", type=float, help="Scale factor (if not provided, will auto-calibrate)")
    parser.add_argument("--calibrate", action="store_true", help="Auto-calibrate scale against current screen")
    parser.add_argument("--serial", default="127.0.0.1:5555", help="ADB serial for calibration")
    parser.add_argument("--screenshot", help="Optional local screenshot path to use instead of adb")
